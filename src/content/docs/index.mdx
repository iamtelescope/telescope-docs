...
If you want to use Telescope with your local `~/.kube/config` file as a Kubernetes source, you have two options:
- **Mount the config file**: Add `-v ~/.kube/config:/root/.kube/config:ro` to the docker run command
- **Copy the config manually**: Copy and paste the contents of your `~/.kube/config` file into Telescope's source configuration UI
:::
